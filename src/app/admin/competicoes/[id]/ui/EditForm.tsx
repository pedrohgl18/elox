...
export default function EditForm({ competition }: { competition: Competition }) {
  ...
  const [audioLinks, setAudioLinks] = useState<Array<{ platform: 'tiktok' | 'instagram' | 'kwai' | 'youtube'; type: 'name' | 'url'; value: string; label?: string }>>(
    (competition.assets?.audioLinks || []).map((a: any) => ({
      platform: a.platform,
      type: a?.url ? 'url' : 'name',
      value: a?.url ?? (a?.label ?? ''),
      label: a?.label,
          ...
          assets: {
            audioLinks: audioLinks.length
              ? audioLinks.map(a => ({
                  ...
                  label: a.label || (a.type === 'name' ? a.value : undefined),
                }))
              : undefined,
          },
          phases,
  ...
  <form onSubmit={onSubmit} className="space-y-4 max-w-5xl">
      ...
      <div className="border border-slate-800 rounded-xl p-4 bg-slate-900/60">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            type="button"
            className="h-9 px-3 rounded-lg border border-slate-700 text-slate-200 hover:bg-slate-800"
          ...
          {audioLinks.map((a, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-[140px_150px_1fr_200px_auto] lg:grid-cols-[160px_180px_1fr_220px_auto] gap-3 items-center">
              <div>
                ...
                <select
                  className="w-full bg-slate-900 text-slate-200 border border-slate-800 rounded-lg h-10 px-3"
                  value={a.platform}
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                    setAudioLinks(rs => rs.map((x,i) => i===idx ? { ...x, platform: e.target.value as any } : x))
                  }
                >
              ...
              <div>
                ...
                <select
                  ...
                  value={a.type}
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                    setAudioLinks(rs => rs.map((x,i) => i===idx ? { ...x, type: e.target.value as 'name' | 'url' } : x))
                  }
                >
              ...
              <div>
                ...
                <input
                  ...
                  value={a.value}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setAudioLinks(rs => rs.map((x,i) => i===idx ? { ...x, value: e.target.value } : x))
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  value={a.label || ''}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setAudioLinks(rs => rs.map((x,i) => i===idx ? { ...x, label: e.target.value } : x))
                  }
                />
              </div>
              <div className="flex items-end">
                <button
                  type="button"
                  className="h-9 px-3 rounded-lg border border-slate-700 text-slate-200 hover:bg-slate-800"
                  onClick={() => setAudioLinks(rs => rs.filter((_,i) => i!==idx))}
                >Remover</button>
              </div>
            </div>
          ))}
