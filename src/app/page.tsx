  ...
  <section id="beneficios" className="w-full max-w-7xl mx-auto grid gap-6 sm:gap-8 md:grid-cols-3 mb-16 px-2 sm:px-4">
      ...
      {active && (
        <section className="w-full max-w-7xl mx-auto mb-8 px-2 sm:px-4">
          <CompetitionBanner name={active.name} endsAt={active.endDate} prize={(active.rewards?.length ? `R$ ${active.rewards.reduce((sum: number, r: any) => sum + (r.amount||0), 0).toLocaleString('pt-BR')}` : '—')} />
        </section>
      )}
