        ...
        <thead className="bg-gradient-to-r from-slate-950 via-emerald-900/10 to-slate-950">
          <tr>
            {columns.map((col, i) => (
              <th
                key={i}
                className="px-3 sm:px-4 py-2.5 sm:py-3 text-left font-semibold text-slate-200 tracking-wide backdrop-blur-sm"
              >
                {col.label}
              </th>
            ))}
          ...
          {data.map((row, i) => (
            ...
            >
              {columns.map((col, j) => (
                <td
                  key={j}
                  className="px-3 sm:px-4 py-2 text-slate-100/90 transition-colors duration-200 group-hover:text-slate-100"
                >
                  {row[col.key]}
                </td>
              ))}
